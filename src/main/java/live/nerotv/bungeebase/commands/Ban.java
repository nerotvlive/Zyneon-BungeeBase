...
import live.nerotv.bungeebase.api.UUIDFetcher;
...
public class Ban extends Command {
    ...
    public void execute(CommandSender s, String[] args) {
        if(s.hasPermission("zyneon.team.ban")) {
            if(args.length == 0) {
                sendSyntax(s);
            } else {
                String r;
                ...
                if(BungeeCord.getInstance().getPlayer(args[0])!=null) {
                    if(BanAPI.isBanned(BungeeCord.getInstance().getPlayer(args[0]).getUniqueId())) {
                        API.sendErrorMessage(s,"§cDieser Spieler ist bereits gebannt§8!");
                        return;
                    }
                    BanAPI.ban(BungeeCord.getInstance().getPlayer(args[0]),r);
                    ...
                    UUID uuid;
                    try {
                        uuid = java.util.UUID.fromString(UUIDFetcher.getUUID(args[0]).replaceFirst("(\\p{XDigit}{8})(\\p{XDigit}{4})(\\p{XDigit}{4})(\\p{XDigit}{4})(\\p{XDigit}+)", "$1-$2-$3-$4-$5"));
                    } catch (Exception e) {
